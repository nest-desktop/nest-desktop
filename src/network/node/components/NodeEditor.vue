  ...
  >
    ...
    >
          ...
          <v-btn-group class="py-1 pr-2" style="width: 100%" variant="text">
            ...
            <v-menu v-model="state.menu" :close-on-content-click="false">
              <template #activator="{ props: btnProps }">
                <v-btn
                  :disabled="node.params.keys.length === 0"
      ...
      <slot name="connectionEditor">
        <ConnectionEditor v-for="(connection, index) in node.connections" :key="index" :connection />
      </slot>
    </v-expansion-panels>
  </Card>
</template>
...
import type { TModel, TNode } from "@/types";
import { Card, Menu, ValueSlider } from "@/components";
