...
export class SpikeCountPlotModel extends SpikeTimesPanelModel {
  ...
  override addData(activity: SpikeActivity): void {
    ...
    if (this.normalization === "min-max scale") {
      const minVal = d3.min(h) as number;
      const maxVal = d3.max(h) as number;
      ...
      const minVal = (d3.min(hh) as number) * ratio;
      const maxVal = (d3.max(hh) as number) * ratio;
      y = h.map((val: number) => (val - minVal) / (maxVal - minVal));
    } else if (this.normalization === "standard score") {
      const mean = d3.mean(h) as number;
      const std = d3.deviation(h) as number;
